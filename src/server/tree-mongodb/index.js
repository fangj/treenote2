...
function buildRootIfNotExist(cb){
  return async(function(){
    ...
    if(!root){
      ...
      root=await(db.insertOne(defaultRoot));
    }
...
function _mk_son_by_data(pNode,data,bgid){
  return async(function(){
    ...
    var result=await(db.insertOne(newNode));//插入新节点
    var _newNode=await(db.findOne({_id:result.insertedId}))
    var pos=0;
    ...
    await(db.update({_id:pNode._id}, pNode, {}));//插入父节点
