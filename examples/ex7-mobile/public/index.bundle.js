...
/***/ function(module, exports, __webpack_require__) {
	  ...
	  _createClass(TreeViewMobile, [{
	    ...
	    value: function render() {
	      ...
	      return _react2.default.createElement(
	        'div',
	        { className: 'tree-container', style: { left: cx, width: W * 3 } },
	        _react2.default.createElement(
	          'div',
	          { className: 'list leftlist', style: { left: 0, width: W } },
	          l
	        ),
	        _react2.default.createElement(
	          'div',
	        ...
	        _react2.default.createElement(
	          'div',
	          { className: 'list rightlist', style: { left: W * 2, width: W } },
	          r
	        )
	      );
	    }
	...
	function panMove(state, _ref, msg, me) {
	  var e = _ref.e,
	      x = _ref.x;

	  var dx = x - me.panStartX;
	  if (Math.abs(dx) > TX) {
	    e.preventDefault();
	  }
	  state.cx = dx;
	...
	function panEnd(state, x, msg, me) {
	  ...
	  if (Math.abs(dx) < TX) {
	    ...
	    var v = (W - Math.abs(dx)) / (panEndTime - me.panStartTime); //计算速度
	    if (dx < -TX) {
	      //左移
	...
	var tweenH = function tweenH(me, state, v) {
	  return new TWEEN.Tween(state).easing(TWEEN.Easing.Quadratic.In).to({ cx: CX0 }, Math.abs(state.cx - CX0) / v).onUpdate(function () {
	    me.setState(state);
	  });
	};
